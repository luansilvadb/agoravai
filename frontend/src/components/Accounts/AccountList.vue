    ...
    >
      ...
      <Column header="Ações" headerStyle="width: 10rem; text-align: center" bodyStyle="text-align: center; overflow: visible">
        <template #body="slotProps">
          ...
          />
          <Button
            v-tooltip.top="'Marcar como paga'"
            icon="pi pi-check"
            class="p-button-rounded p-button-info mr-2"
            @click="confirmPayAccount(slotProps.data)"
            :disabled="slotProps.data.status === 'paid' || loading"
          />
          <Button
...
import { getAccounts, deleteAccount, markAsPaid } from '@/api/accountsPayable';
...
const loadAccounts = async () => {
  try {
    ...
    const response = await getAccounts({ 
      ...
      dateFrom,
      dateTo,
      status: filters.value.status as 'pending' | 'paid' | 'overdue' | undefined
    });
    ...
    toast.add({
      severity: 'error',
      summary: 'Erro',
      detail: 'Falha ao carregar as contas a pagar',
      life: 5000
    });
  } finally {
    loading.value = false;
...
const confirmPayAccount = (account: any) => {
  confirm.require({
    message: `Tem certeza que deseja marcar a conta "${account.description}" como paga?`,
    header: 'Confirmação',
    icon: 'pi pi-exclamation-triangle',
    accept: () => {
      payAccountAction(account.id);
    }
  });
};

const payAccountAction = async (accountId: number) => {
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Erro',
      detail: error.response?.data?.error?.message || 'Falha ao marcar conta como paga',
      life: 5000
    });
  }
};
