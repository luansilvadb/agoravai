<template>
  <Dialog 
    ...
    :header="isEditing ? 'Editar Conta a Receber' : 'Nova Conta a Receber'" 
    :modal="true" 
    :closable="true"
    :style="{ width: '500px' }"
    @hide="resetForm"
...
import { createReceivableAccount, updateReceivableAccount } from '@/api/accountsReceivable';
...
const categoryForm = ref({
  name: '',
  type: 'income' as 'expense' | 'income' // Padrão para receita
});
...
const resetCategoryForm = () => {
  categoryForm.value = {
    name: '',
    type: 'income' // Padrão para receita
  };
...
const searchCategories = (event: any) => {
  if (!event.query.trim()) {
    // Filtrar apenas categorias de receita para contas a receber
    ...
    filteredCategories.value = categoryStore.categories
      .filter(cat => 
        cat.name.toLowerCase().includes(event.query.toLowerCase()) && 
        cat.type === 'income' // Apenas categorias de receita
      )
      .map(cat => ({
        ...cat,
        typeLabel: cat.type === 'expense' ? 'Despesa' : 'Receita'
  ...
  const hasExactMatch = categoryStore.categories.some(
    cat => cat.name.toLowerCase() === event.query.toLowerCase() && cat.type === 'income'
  );
...
const openCategoryCreation = () => {
  ...
  categoryForm.value.type = 'income'; // Padrão para receita
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.isEditing && props.accountData?.id) {
      ...
      account = await updateReceivableAccount(props.accountData.id, {
      ...
      // Criar nova conta
      account = await createReceivableAccount({
        description: formData.value.description,
        amount: formData.value.amount,
        dueDate: formData.value.dueDate.toISOString(),
        categoryId: formData.value.categoryId
    ...
    if (error.response?.data?.error) {
      ...
      toast.add({
        severity: 'error',
        summary: 'Erro',
        detail: 'Falha ao salvar a conta a receber',
        life: 5000
      });
    }
  } finally {
