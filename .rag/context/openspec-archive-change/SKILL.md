...
version: 1.0.0
description: >
  Arquiva uma change concluída no workflow experimental do openspec.
...
triggers:
  ...
  - "/archive-change"
scope:
  primary: ["openspec", "change lifecycle", "archive"]
  delegates: ["openspec-sync-specs para sincronização de specs", "openspec-create-change para criação de nova change"]
quality_bar: high
---

# OPENSPEC ARCHIVE CHANGE — Arquivamento seguro de changes concluídas
...
1. **Nunca auto-selecionar** — O usuário escolhe a change. Inferência contextual é aceita; seleção automática não.
   Na prática: se o nome não for explícito ou unívoco, sempre execute `openspec list` e apresente seleção.
...
3. **Preservar estado original** — Arquivamento move, não copia. O diretório deve permanecer intacto no destino.
   Na prática: use `mv` atômico, nunca `cp + rm`, e valide que `.openspec.yaml` acompanhou.
...
### ❌ NÃO ativar para:
- Criar nova change → use `openspec-create-change`
- Sincronizar specs sem arquivar → use `openspec-sync-specs`
- Listar changes sem intenção de arquivar → responda diretamente com `openspec list`
- Reverter arquivamento → responda diretamente com `mv openspec/changes/archive/<name> openspec/changes/<name>`
...
|---|---|
| Seleção de change via prompt | Sincronização de delta specs → `openspec-sync-specs` |
| Validação de artefatos via CLI | Criação de nova change → `openspec-create-change` |
| Validação de tarefas via filesystem | Modificação de specs manuais → edição direta |
| Avaliação de delta specs (leitura) | Resolução de conflitos de sync → `openspec-sync-specs` |
...
1.2. Validar que o diretório openspec/changes/<name> existe.
...
```
1.1. Executar: openspec list --json
1.2. [RESILIÊNCIA] Se o comando falhar:
     - Erro de CLI não encontrado: falhar com "openspec CLI não está instalado ou não está no PATH"
     - JSON inválido: falhar com "Output de openspec list não é JSON válido. Stderr: <stderr>"
     - Exit code != 0: falhar com "openspec list retornou erro (exit <code>): <stderr>"
1.3. Filtrar a lista para changes ativas (excluir as que estão em openspec/changes/archive/).
...
```
2.1. Executar: openspec status --change "<CHANGE_NAME>" --json
2.2. [RESILIÊNCIA] Se o comando falhar:
     - Change não encontrada: falhar com "Change '<CHANGE_NAME>' não encontrada no openspec"
     - JSON inválido: falhar com "Output de openspec status não é JSON válido. Stderr: <stderr>"
     - Exit code != 0: falhar com "openspec status retornou erro (exit <code>): <stderr>"
2.3. Parsear o JSON:
     - Extrair schemaName
...
```
3.1. Construir caminho: openspec/changes/<CHANGE_NAME>/tasks.md
...
```
4.1. Construir caminho: openspec/changes/<CHANGE_NAME>/specs/
4.2. [CASO DE EXCEÇÃO] Se o diretório não existe ou está vazio:
     - Armazenar flag HAS_DELTA_SPECS=false
     - Prosseguir para PASSO 5.
4.3. Listar todos os arquivos .md em openspec/changes/<CHANGE_NAME>/specs/ (recursivo)
...
4.6. Para cada delta spec encontrado:
     ...
     4.6.2. Construir caminho do main spec: openspec/specs/<capability>/spec.md
...
4.10. [SYNC] Invocar sincronização:
      ...
      - Prompt: "Use Skill tool to invoke openspec-sync-specs for change '<CHANGE_NAME>'. Delta spec analysis: <resumo do passo 4.7>"
...
5.4. Construir caminho completo: openspec/changes/archive/<DEST_NAME>
5.5. [CASO DE EXCEÇÃO] Se destino já existe:
     - Falhar com erro explícito:
       "Já existe um archive com este nome: openspec/changes/archive/<DEST_NAME>/
        Opções:
...
5.6. Executar movimentação:
     mv openspec/changes/<CHANGE_NAME> openspec/changes/archive/<DEST_NAME>
...
5.8. Validar que o destino existe e contém .openspec.yaml:
     - Se destino não existe após mv: falhar com "Move aparentemente sucedeu mas destino não encontrado. Possível race condition."
     - Se .openspec.yaml não existe no destino: warning "Arquivo .openspec.yaml não encontrado no archive. Verifique integridade manualmente."
5.9. Prosseguir para PASSO 6.
```

**Critério de conclusão**: Diretório movido com sucesso para `openspec/changes/archive/<DEST_NAME>/`.
...
**Change:** <CHANGE_NAME>
**Schema:** <SCHEMA_NAME>
**Archived to:** openspec/changes/archive/<DEST_NAME>/
...
### P1 — Parsing de JSON do openspec status
...
STATUS_JSON=$(openspec status --change "$NAME" --json 2>/tmp/openspec-err)
EXIT_CODE=$?
if [ $EXIT_CODE -ne 0 ]; then
  echo "ERRO: openspec status falhou (exit $EXIT_CODE): $(cat /tmp/openspec-err)"
  exit 1
fi
...
**Por que importa**: O openspec pode mudar formato de output, ter bugs, ou retornar erros em stdout. Acesso sem validação propaga "null" silenciosamente pelo resto do fluxo.
...
mv "openspec/changes/$NAME" "openspec/changes/archive/$DEST" 2>/tmp/mv-err
if [ $? -ne 0 ]; then
  echo "ERRO: mv falhou: $(cat /tmp/mv-err)"
  exit 1
fi
if [ ! -d "openspec/changes/archive/$DEST" ]; then
  echo "ERRO CRÍTICO: mv retornou sucesso mas destino não existe"
  exit 1
fi
if [ ! -f "openspec/changes/archive/$DEST/.openspec.yaml" ]; then
  echo "WARNING: .openspec.yaml ausente no archive"
fi

# ❌ FAIL — cp + rm sem validação
cp -r "openspec/changes/$NAME" "openspec/changes/archive/$DEST"
...
### P5 — Tratamento de stderr do openspec CLI

**Regra**: Sempre capturar stderr separadamente. Nunca mesclar stderr com stdout para parsing.

```bash
# ✅ PASS — Stderr separado
OUTPUT=$(openspec list --json 2>/tmp/openspec-stderr)
EXIT_CODE=$?
if [ $EXIT_CODE -ne 0 ]; then
  echo "ERRO: $(cat /tmp/openspec-stderr)"
  exit 1
fi

# ❌ FAIL — Stderr no mesmo stream
OUTPUT=$(openspec list --json 2>&1)
...
| Ignorar stderr do openspec CLI | JSON corrompido por mensagens de warning, parse falha silenciosamente | Capturar stderr separado, verificar exit code |
...
| Não validar pós-move | Diretório pode ter sumido por race condition com outro processo | Validar existência + presença de .openspec.yaml |
...
- [ ] `openspec status --json` foi parseado com validação de estrutura
...
- [ ] Arquivo .openspec.yaml acompanhou a change para o archive
...
|---|---|
| Sincronizar delta specs com main specs | `openspec-sync-specs` |
| Criar uma nova change | `openspec-create-change` |
| Listar changes disponíveis | `openspec` CLI diretamente (`openspec list`) |
| Verificar status detalhado | `openspec` CLI diretamente (`openspec status`) |
...
[CLI]: openspec status --change "auth-refactor" --json
[Output]: {"schemaName": "default", "artifacts": [{"name": "plan", "status": "done"}, {"name": "impl", "status": "done"}]}

[Modelo]: ✓ Todos os 2 artefatos completos.

[Filesystem]: tasks.md não encontrado em openspec/changes/auth-refactor/
[Modelo]: Nenhum arquivo de tarefas encontrado.

[Filesystem]: openspec/changes/auth-refactor/specs/ não encontrado
[Modelo]: Nenhum delta spec para sincronizar.

[CLI]: mv openspec/changes/auth-refactor openspec/changes/archive/2025-01-15-auth-refactor
...
**Schema:** default
**Archived to:** openspec/changes/archive/2025-01-15-auth-refactor/
...
[CLI]: openspec status --change "api-v2" --json
...
[Filesystem]: openspec/changes/api-v2/specs/ contém:
...
[Task tool]: Invoca openspec-sync-specs para api-v2...
[Resultado]: Sync concluído com sucesso

[CLI]: mv openspec/changes/api-v2 openspec/changes/archive/2025-01-15-api-v2
...
**Change:** api-v2
**Schema:** extended
**Archived to:** openspec/changes/archive/2025-01-15-api-v2/
...
[CLI]: openspec list --json
[Exit]: 127
[Stderr]: openspec: command not found

[Modelo]:
## ERRO

openspec CLI não está instalado ou não está no PATH.

**Ação sugerida**: Verifique a instalação do openspec com `which openspec` ou consulte a documentação de instalação.
```

### Cenário D — Destino de archive já existe

```
[CLI]: ls openspec/changes/archive/2025-01-15-auth-refactor
[Output]: (diretório existe)

[Modelo]:
## ERRO

Já existe um archive com este nome: openspec/changes/archive/2025-01-15-auth-refactor/

**Opções:**
- Renomear o archive existente manualmente: `mv openspec/changes/archive/2025-01-15-auth-refactor openspec/changes/archive/2025-01-15-auth-refactor-v2`
