...
3. **Status:** Run `npm run specskill:status -- --change "<name>" --json`. Extract `applyRequires` and `artifacts`.
4. **Artifact Loop:** Use `TodoWrite`. Loop until all IDs in `applyRequires` are `status: "done"`:
   ...
   b. Run `npm run specskill:instructions -- <artifact-id> --change "<name>" --json`.
   ...
   g. Re-run `npm run specskill:status -- --change "<name>" --json` to update loop condition.
   h. If critically unclear, use `AskUserQuestion`.
5. **Final:** Run `npm run specskill:status -- --change "<name>"`.
