...
### AC 3.4.1: Basic List Display
**Given** an Admin or Operational user accesses `/accounts-payable`
**When** the page loads
...
### AC 3.4.2: Filtering by Period
**Given** the user applies date filters (from/to) and/or status (pending/paid/overdue)
...
### AC 3.4.3: Overdue Status Display
**Given** an account is overdue (`due_date` < today and status "pending")
**When** it appears in the list
**Then** the status displays a red "Vencida" badge

### AC 3.4.4: API Endpoints for Filtered Queries
**Given** an authenticated request to payable accounts endpoint with filters
...
- [x] Implement backend filtered querying functionality (AC: 3.4.1, 3.4.2, 3.4.3, 3.4.4)
  - [x] Enhance accountService.ts with filtered querying function
  - [x] Update accounts-payable route with filtering parameters support
...
- [x] Implement frontend filtered list interface (AC: 3.4.1, 3.4.2, 3.4.3, 3.4.5)
  - [x] Update AccountList.vue with filter controls (date pickers, status dropdown)
...
- [x] Create comprehensive tests (AC: All)
  - [x] Unit test for filtered querying service function
  - [x] Integration test for filtered accounts endpoint
  - [x] Test tenant isolation for filtered queries
  - [x] Test RBAC enforcement for filtered queries
  - [x] Test various filter combinations
  - [x] Test pagination with filters
  - [x] Test audit logging for filtered queries
...
- The AccountList.vue component already exists but needs filtering capabilities

### References

- [Source: _bmad-output/planning-artifacts/epics.md#Story 3.4: Lista de Contas a Pagar com Filtros]
...
- Filtered querying service tests
- Tenant isolation tests for filtered queries
- RBAC enforcement tests for filtered queries
- Various filter combinations tests
- Pagination with filters tests
- Frontend component tests with filtering

### Completion Notes List

- Successfully implemented filtering functionality for payable accounts list with date range and status filters
- Backend changes: Enhanced accountService.ts with filtered querying function supporting date range and status filters, implemented efficient database queries with proper WHERE clauses
- Frontend changes: Enhanced AccountList.vue with comprehensive filter controls (date pickers, status dropdown), implemented real-time filtering, added proper BRL currency formatting and colorful status badges
- Testing: Enhanced existing tests to cover filtering scenarios
