...
- **Global scope**: `D:/agoravai/.windsurf/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/agoravai/.windsurf/notes/`)
...
2. Glob `D:/agoravai/.windsurf/notes/*.md` (if directory exists) — global notes
...
Global (D:/agoravai/.windsurf/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/agoravai/.windsurf/notes/` — works in any directory
